...
metadata <- subset(metadata_file, Sample_ID_16S!="nd")
...
product_variable = as.character(get_variable(bacdata, "Product"))
time_point_variable = as.character(get_variable(bacdata, "Time_point"))
sample_data(bacdata)$dereplicated <- mapply(paste, product_variable, time_point_variable)

#applying correct order to phyloseq object
sample_data(bacdata)$Time_point=factor(sample_data(bacdata)$Time_point, levels = c("pre-inoculation", "post-inoculation", "early", "middle", "end", "packaged"))
sample_data(bacdata)$dereplicated=factor(sample_data(bacdata)$dereplicated, levels = c("soy pre-inoculation", "soy post-inoculation", "soy early", "soy middle", "soy end", "soy packaged", "multigrain pre-inoculation", "multigrain post-inoculation", "multigrain early", "multigrain middle", "multigrain end", "multigrain packaged"))
sample_data(bacdata)$Product=factor(sample_data(bacdata)$Product, levels = c("soy", "multigrain"))
#######################################################
...
write.csv(summary_table(bacdata), "Bacdata_OTU_table.csv", quote=F)
#######################################################


#Filtering for bacteria, chloroplast, and mitochondria
#######################################################
#filtering features
...
Samples <- subset_samples(bacdataFilt, TRUE_or_CONTROL=="TRUE SAMPLE")
Ext_controls <- subset_samples(bacdataFilt, Sample_ID_16S=="Ext_neg_1_1_16S" | Sample_ID_16S=="Ext_neg_1_2_16S")
PCR_controls <- subset_samples(bacdataFilt, Sample_ID_16S=="PCR_neg_1_16S" | Sample_ID_16S=="PCR_neg_2_16S")
...
contam_Ext <- isContaminant(decontam_Ext, batch="X16S_Amplification_Batch", method="prevalence", neg="is.neg")
...
#adding some metadata to the table  
  ...
  samples_data <- t(sample_data(Samples)[, c("Product", "Time_point", "X16S_Amplification_Batch")])
  Ext_data <- t(sample_data(Ext_controls)[, c("Product", "Time_point", "X16S_Amplification_Batch")])
...
contam_table_Ext[row.names(subset(contam_table_Ext, Genus=="Escherichia-Shigella")), 'contaminant'] <- T #after looking at data these these appeared to be contaminants
contam_table_Ext['ffe1e63147af5c2cd21231f1d5d59488', 'contaminant'] <- T #after looking at data this appeared to be a contaminant
contam_table_Ext['b02a8d33d018119dedb2db15db887bfd', 'contaminant'] <- T #after looking at data this appeared to be a contaminant
...
contam_PCR <- isContaminant(decontam_PCR, batch="X16S_Amplification_Batch", method="prevalence", neg="is.neg", threshold=0.5) #different threshold chosen after looking at data
...
#contam_table_PCR <- subset(contam_table_PCR, contam_table_PCR[,"PCR_neg_1_16S"]!=0 | contam_table_PCR[,"PCR_neg_2_16S"]!=0)
  #adding some metadata to the table  
  ...
  samples_data <- t(sample_data(Samples)[, c("Product", "Time_point", "X16S_Amplification_Batch")])
  PCR_data <- t(sample_data(PCR_controls)[, c("Product", "Time_point", "X16S_Amplification_Batch")])
  blank <- matrix(rep(NA, 13*3), ncol=13)
...
contam_table_PCR[row.names(subset(contam_table_PCR, Genus=="Escherichia-Shigella")), 'contaminant'] <- T #after looking at data these these appeared to be contaminants
...
bacdata_decontam <- subset_samples(bacdata_decontam, TRUE_or_CONTROL=="TRUE SAMPLE")
...
bacdata_decontamGlomFamily <- glom_tax(samples_plotting, "Family")
bacdata_decontamGlomFamilyNorm <- transform_sample_counts(bacdata_decontamGlomFamily, function(x) x / sum(x))

plotting <- bacdata_decontamGlomFamilyNorm
...
#combined_rel_abundance_plot <- annotate_figure(combined_rel_abundance_plot, top = text_grob("Bacterial Family Relative Abundance", face = "bold", family="serif", size=18))
#exported as tiff Bacterial_Relative_Abundance
ggsave(plot=combined_rel_abundance_plot, filename="Bacterial_Relative_Abundance.tiff", width=8, height=6, units="in", dpi="print")
#######################################################
...
bacdata_decontam_nostart <- subset_samples(bacdata_decontam, Time_point!="pre-inoculation" & Time_point!="post-inoculation")
  bacdata_decontam_nostart <- filter_taxa(bacdata_decontam_nostart, function(x) sum(x)!=0, prune=T)
...
#splitting up bacdata_decontam_nostart object, as the otu table will be modified and the separate parts will need to be merged into a new object after denoising
bacdatadecontam_otu <- otu_table(bacdata_decontam_nostart)
bacdatadecontam_tax <- tax_table(bacdata_decontam_nostart)
bacdatadecontam_sample <- sample_data(bacdata_decontam_nostart)
bacdatadecontam_seqs <- refseq(bacdata_decontam_nostart)
...
#table summarizing number of filtered reads
sum_table_denoised <- cbind(sum_table[sample_names(bacdata_denoised),], data.frame(sample_sums(bacdata_denoised)))
colnames(sum_table_denoised) <- c("total unfiltered", "non-bacteria", "unidentified at phylum", "chloroplast", "mitochondria", "passing filters", "contaminants removed/clean", "denoised")
sum_table_denoised
...
bacdata_diversity <- alpha(phyloseq4alpha, index=c("inverse_simpson", "shannon", "coverage", "pielou", "observed", "chao1", "relative", "core_abundance", "low_abundance"))
indices <- names(bacdata_diversity)
...
bacdata_diversity <- cbind(sample_data(phyloseq4alpha), bacdata_diversity)
bacdata_diversity$Time_point=factor(bacdata_diversity$Time_point, levels = c("pre-inoculation", "post-inoculation", "early", "middle", "end", "packaged"))
bacdata_diversity$dereplicated=factor(bacdata_diversity$dereplicated, levels = c("soy pre-inoculation", "soy post-inoculation", "soy early", "soy middle", "soy end", "soy packaged", "multigrain pre-inoculation", "multigrain post-inoculation", "multigrain early", "multigrain middle", "multigrain end", "multigrain packaged"))
bacdata_diversity$Product=factor(bacdata_diversity$Product, levels = c("soy", "multigrain"))
#bacdata_diversity is a data frame holding all the alpha diversity calculations

measures <- c("Product", "Time_point", "dereplicated")
plots <- c()
stats <- c()
for(i in indices){
  plots[[i]] <- ggboxplot(bacdata_diversity, x="Product", y=i, fill="Product", palette="simpsons", ylab=index_names[i], xlab=F, add="point", add.params=list(color="darkgrey"))+rremove("x.text")+theme(text=element_text(family="serif"))
  pvals <- c()
for(m in measures){
    stats[[i]][[m]] <- kruskal.test(unlist(bacdata_diversity[i]), unlist(bacdata_diversity[m]))
...
#  combined_alpha_diversity_plot <- annotate_figure(combined_alpha_diversity_plot, top=text_grob("Bacterial Alpha Diversity Product Comparison", face="bold", size=18, family="serif"))
#exported as tiff Bacterial_Alpha_Diversity_Product
ggsave(plot=combined_alpha_diversity_plot, filename="Bacterial_Alpha_Diversity.tiff", width=8, height=6, units="in", dpi="print")
#######################################################
...
#  labs(title="Bacterial Species Bray-Curtis Ordination")+
#  theme(plot.title=element_text(family="serif", face="bold", size=18))
#exported as tiff Bacterial_Bray_Ordination
ggsave(plot=combined_ordination_plot, filename="Bacterial_Bray_Ordination.tiff", width=8, height=6, units="in", dpi="print")
#######################################################
...
top_coef_Product <- as.data.frame(coef_Product[rev(order(abs(coef_Product)))[1:10]])
...
plot_coef_Product <- ggbarplot(data=top_coef_Product, x="species_name", y="coefficient", fill="coefficient", orientation="horiz", legend="none")+
  scale_fill_gradient(low="#FEDD61", high="#9BB8EA", limits=c(-0.06, 0.06))+
...
ggsave(plot=plot_coef_Product, filename="Bacterial_Product_PERMANOVA_Coefficients.tiff", width=8, height=3, units="in", dpi="print")
#######################################################
...
write.csv(res_product, "16S_ZicoSeq_Product.csv", quote=F)
...
write.csv(res_Time_point, "16S_ZicoSeq_Time_point.csv", quote=F)
#######################################################
...
unique_middle <- tax_table(phyloseq_core_time)[setdiff(c(rownames(tax_core_time$middle)), c(rownames(tax_core_time$early)))]
unique_earlymiddle <- tax_table(phyloseq_core_time)[setdiff(intersect(rownames(tax_core_time$middle), rownames(tax_core_time$early)), rownames(time_intersect))]
unique_packaged <- tax_table(phyloseq_core_time)[setdiff(rownames(tax_core_time$packaged), rownames(time_intersect))]
time_coloring <- rev(c("red", "red", "red", "red", "red", "red", "red", "red", "blue", "purple", "darkgreen", "black"))
###################################################
...
venn_plot_time <- ggplot()+
  ...
  geom_sf_label(aes(label=count), data=subset(venn_region(data_time), count>0), fontface="bold", family="serif", alpha=1, color=c("purple", "darkgreen", "blue", "red"))+ # region labels
...
unique_multigrain <- tax_table(phyloseq_core_product)[setdiff(rownames(tax_core_product$multigrain), rownames(product_intersect))]
unique_soy <- tax_table(phyloseq_core_product)[setdiff(rownames(tax_core_product$soy), rownames(product_intersect))]
product_coloring <- rev(c("red", "red", "red", "blue", "red", "red", "red", "red", "blue", "darkgreen", "darkgreen", "darkgreen", "darkgreen", "darkgreen", "darkgreen", "black"))
###################################################
...
venn_plot_product <- ggplot()+
  ...
  geom_sf_label(aes(label=count), data=subset(venn_region(data_product), count>0), fontface="bold", family="serif", alpha=1, color=c("blue", "darkgreen", "red"))+ # region labels
  scale_fill_gradient(low="#f8fafe", high="#7EA4E4", limits=c(0,10))+
...
#  labs(title="Core Bacterial Species")+

#Exported as tiff Bacterial_Core
ggsave(plot=combined_core_plot, filename="Bacterial_Core.tiff", width=8, height=6, units="in", dpi="print")
#######################################################


#Minorly looking at taxa from starting samples
#######################################################
bacdata_start <- subset_samples(bacdata_decontam, Time_point=="pre-inoculation" | Time_point=="post-inoculation")
  bacdata__start <- filter_taxa(bacdata_start, function(x) sum(x)!=0, prune=T)

#aglommerating data to Family level for plotting
bacdata_startGlomSpecies <- glom_tax(bacdata__start, "Species")
bacdata_startGlomSpeciesNorm <- transform_sample_counts(bacdata_startGlomSpecies, function(x) x / sum(x))

summary_table(bacdata_startGlomSpeciesNorm)
df <- as.data.frame(summary_table_bacdata_startGlomSpeciesNorm[8:19])
...
summary_table_bacdata_startGlomSpeciesNorm <- cbind(summary_table_bacdata_startGlomSpeciesNorm, mean, max, prevalence)

write.csv(summary_table_bacdata_startGlomSpeciesNorm, "Bacdata_start.csv", quote=F)
